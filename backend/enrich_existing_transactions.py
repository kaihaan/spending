...
def enrich_transactions(category='Other', limit=50, force_refresh=False, verbose=True):
    ...
    try:
        ...
        if verbose:
            ...
            print(f"🔍 Found {len(transactions_to_enrich)} transactions matching filter ({filter_msg})")

        # Apply limit
        transactions_to_enrich = transactions_to_enrich[:limit]

        if not transactions_to_enrich:
            print(f"⚠️  No transactions found to enrich.")
            return None
...
def show_stats(verbose=True):
    ...
    try:
        ...
        enrichment_pct = round((enriched_count / len(all_transactions) * 100) if all_transactions else 0, 1)
        ...
        for cat in sorted(category_counts.keys()):
            ...
            pct = round((count / len(all_transactions) * 100) if all_transactions else 0, 1)
