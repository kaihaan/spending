...
def match_all_amazon_business_transactions() -> dict:
    ...
    unmatched_transactions = database.get_unmatched_truelayer_amazon_business_transactions()
    print(f"[Amazon Business Matcher] Found {len(unmatched_transactions)} unmatched Amazon transactions")
    ...
    print(f"[Amazon Business Matcher] Found {len(all_orders)} Amazon Business orders in database")

    if not all_orders:
        print("[Amazon Business Matcher] WARNING: No Amazon Business orders in database to match against!")
        return {
    ...
    for transaction in unmatched_transactions:
        print(f"[Amazon Business Matcher] Checking bank txn: date={transaction.get('timestamp')}, "
        ...
        if match:
            print(f"[Amazon Business Matcher] MATCH FOUND: order_id={match['order_id']}, "
            ...
            print(f"[Amazon Business Matcher] No match found for this transaction")

    print(f"[Amazon Business Matcher] RESULT: {matched_count} matched, "
...
def find_best_amazon_business_match(transaction: dict, orders: list) -> dict:
    ...
    try:
        ...
        if isinstance(txn_timestamp, datetime):
            ...
            txn_date = datetime.combine(txn_timestamp, datetime.min.time(), tzinfo=tz.utc)
        else:
            date_str = str(txn_timestamp).strip()
            date_part = date_str.split(' ')[0] if ' ' in date_str else date_str.split('T')[0]
            txn_date = datetime.strptime(date_part, '%Y-%m-%d').replace(tzinfo=tz.utc)
    except Exception as e:
        print(f"[Amazon Business Matcher] Failed to parse bank transaction date: {e}")
        return None

    txn_amount = abs(transaction['amount'])
    print(f"[Amazon Business Matcher]   Bank txn parsed: date={txn_date.date()}, amount=£{txn_amount:.2f}")
    ...
    if not ('AMAZON' in merchant or 'AMAZON' in description):
        print(f"[Amazon Business Matcher]   Skipped: Not an Amazon transaction")
        return None

    # Skip consumer marketplace transactions (handled by amazon_matcher.py)
    if 'AMZN MKTP' in description:
        print(f"[Amazon Business Matcher]   Skipped: Consumer marketplace transaction")
        return None

    # Find candidates
    candidates = []
    print(f"[Amazon Business Matcher]   Searching {len(orders)} Amazon Business orders for match...")
        ...
        try:
            ...
            if isinstance(order_date, datetime):
                if order_date.tzinfo is None:
                    order_date = order_date.replace(tzinfo=tz.utc)
            elif isinstance(order_date, type(datetime.now().date())):
                order_date = datetime.combine(order_date, datetime.min.time(), tzinfo=tz.utc)
            else:
                order_date = datetime.strptime(str(order_date), '%Y-%m-%d').replace(tzinfo=tz.utc)
        except Exception:
            continue
    ...
    if not candidates:
        ...
        print(f"[Amazon Business Matcher]   No candidates found. Sample Amazon Business orders:")
        for order in orders[:3]:  # Show first 3
            net_total = float(order.get('net_total') or 0)
            print(f"[Amazon Business Matcher]     Order: date={order.get('order_date')}, "
    ...
    best = candidates[0]
    print(f"[Amazon Business Matcher]   Best candidate: {str(best['product_summary'])[:30]}, "
