...
class ImportJob:
        ...
        try:
            ...
            if self.from_date and self.to_date:
                ...
                to_date_str = (
                    ...
                    if isinstance(self.to_date, date)
                    else self.to_date
                )
            ...
            logger.info(
                f"✓ Plan complete: {estimated_transactions} txns, {estimated_duration:.0f}s, ${estimated_cost:.2f}"
            )
            ...
            return {
                ...
                "date_range": {"from": self.from_date, "to": self.to_date},
                "accounts": [
                    {"id": a["id"], "display_name": a["display_name"]} for a in accounts
                ],
            }
    ...
    def execute(self, use_parallel: bool = False, max_workers: int = 3) -> dict:
        ...
        try:
            # Mark as running
            estimated_time = datetime.now(UTC).isoformat()
            database.update_import_job_status(
                self.job_id, "running", estimated_completion=estimated_time
            )
            ...
            logger.info(
                f"✅ Job complete: {total_synced} synced, {total_duplicates} dupes, {total_errors} errors"
            )
            ...
            database.update_import_job_status(
                self.job_id, "failed", error_message=str(e)
            )
            raise
    ...
    def _get_accounts_to_sync(self) -> list[dict]:
        ...
        if self.account_ids:
            accounts = [a for a in all_accounts if a["account_id"] in self.account_ids]
        else:
    ...
    def get_status(self) -> dict:
        ...
        completed_accounts = sum(
            1 for p in progress if p["progress_status"] == "completed"
        )
        total_accounts = len(progress)

        percent = (
            round(100.0 * completed_accounts / total_accounts)
            if total_accounts > 0
            else 0
        )
