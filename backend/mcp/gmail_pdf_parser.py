...
def parse_date_text(text: str) -> Optional[str]:
    ...
    match = re.search(r'(\d{1,2})\s+(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{4})', text, re.IGNORECASE)
    if match:
...
def parse_charles_tyrwhitt_pdf(pdf_bytes: bytes) -> Optional[dict]:
    """
    ...
    date_match = re.search(r'(?:Date|Receipt Date)[:\s]*(\d{1,2}[/\-]\d{1,2}[/\-]\d{4}|\d{1,2}\s+\w+\s+\d{4})', text, re.IGNORECASE)
    if date_match:
    ...
    # Extract subtotal
    subtotal_match = re.search(r'Sub\s*-?\s*total[:\s]*£\s*([0-9,]+\.?\d*)', text, re.IGNORECASE)
    if subtotal_match:
        result['subtotal'] = parse_amount(subtotal_match.group(1))

    # Extract delivery cost
    delivery_match = re.search(r'(?:Delivery|Shipping)[:\s]*£\s*([0-9,]+\.?\d*)', text, re.IGNORECASE)
    if delivery_match:
...
def parse_google_cloud_pdf(pdf_bytes: bytes) -> Optional[dict]:
    """
    ...
    period_match = re.search(r'(?:Billing Period|Service Period|Summary for)[:\s]*(.+?)(?:\n|$)', text, re.IGNORECASE)
    if period_match:
        result['billing_period'] = period_match.group(1).strip()

    # Extract invoice date - handle "30 Nov 2024" format
    date_match = re.search(r'Invoice\s+date[:\s]*(\d{1,2}\s+\w+\s+\d{4}|\d{1,2}[/\-]\d{1,2}[/\-]\d{4})', text, re.IGNORECASE)
    if date_match:
        result['receipt_date'] = parse_date_text(date_match.group(1))
    ...
    # Extract VAT/Tax
    vat_match = re.search(r'(?:VAT|Tax)[:\s]*[£$€]\s*([0-9,]+\.?\d*)', text, re.IGNORECASE)
    if vat_match:
        result['vat_amount'] = parse_amount(vat_match.group(1))
...
def parse_xero_invoice_pdf(pdf_bytes: bytes) -> Optional[dict]:
    """
    ...
    from_match = re.search(r'From\s+([A-Z][A-Z\s&\-\.]+(?:LTD|LIMITED|PLC|INC|LLC)?)', text, re.IGNORECASE)
    if from_match:
        company = from_match.group(1).strip()
        result['merchant_name'] = company
        result['merchant_name_normalized'] = re.sub(r'[^a-z0-9]+', '_', company.lower()).strip('_')
    else:
    ...
    if invoice_match:
        ...
        # Fallback patterns
        for pattern in [r'Invoice\s*#?\s*([A-Z0-9\-]+)', r'Reference[:\s]*([A-Z0-9\-]+)']:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
    ...
    # Extract date
    date_match = re.search(r'(?:Invoice Date|Date)[:\s]*(\d{1,2}[/\-]\d{1,2}[/\-]\d{4}|\d{1,2}\s+\w+\s+\d{4})', text, re.IGNORECASE)
    if date_match:
        result['receipt_date'] = parse_date_text(date_match.group(1))
    ...
    # Extract VAT/Tax
    vat_match = re.search(r'(?:VAT|Tax)\s*(?:\d+%)?[:\s]*[£$€]?\s*([0-9,]+\.?\d*)', text, re.IGNORECASE)
    if vat_match:
        result['vat_amount'] = parse_amount(vat_match.group(1))
    ...
    for match in re.finditer(r'Total\s+(?:GBP|USD|EUR)\s*([0-9,]+\.?\d*)', text, re.IGNORECASE):
    ...
    if not total_amounts:
        for match in re.finditer(r'Total[:\s]*[£$€]\s*([0-9,]+\.?\d*)', text, re.IGNORECASE):
            amount = parse_amount(match.group(1))
            if amount and amount > 0:
                total_amounts.append(amount)
    ...
    if not total_amounts:
        for match in re.finditer(r'(?:Amount\s+Due|Balance\s+Due)[:\s]*[£$€]?\s*([0-9,]+\.?\d*)', text, re.IGNORECASE):
            amount = parse_amount(match.group(1))
            if amount and amount > 0:
                total_amounts.append(amount)
    ...
    for match in re.finditer(item_pattern, text):
        ...
        skip_keywords = ['subtotal', 'total', 'vat', 'tax', 'discount', 'description',
                        'quantity', 'unit price', 'amount', 'due date', 'invoice']
...
def parse_atlassian_invoice_pdf(pdf_bytes: bytes) -> Optional[dict]:
    """
    ...
    if invoice_match:
        ...
        # Fallback patterns
        for pattern in [r'Invoice\s*#?\s*:?\s*([A-Z0-9\-]+)', r'Invoice Number[:\s]*([A-Z0-9\-]+)']:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                result['order_id'] = match.group(1)
                break
...
def parse_generic_receipt_pdf(pdf_bytes: bytes, merchant_hint: str = None) -> Optional[dict]:
    """
    ...
    for match in re.finditer(item_pattern, text):
        ...
        skip_terms = ['total', 'subtotal', 'vat', 'tax', 'discount', 'shipping',
                     'delivery', 'qty', 'quantity', 'price', 'amount', 'description']
    ...
    if not line_items:
        ...
        for match in re.finditer(simple_pattern, text):
            item_name = match.group(1).strip()
            skip_terms = ['total', 'subtotal', 'vat', 'tax', 'discount', 'shipping',
                         'delivery', 'amount', 'price', 'date', 'invoice', 'receipt']
            if any(term in item_name.lower() for term in skip_terms):
                continue
    ...
    if not line_items:
        product_pattern = r'(?:product|item|service|description)[:\s]*([A-Za-z][A-Za-z0-9\s\-\.\,]+)'
...
def parse_suffolk_latch_pdf(pdf_bytes: bytes) -> Optional[dict]:
    """
    ...
    invoice_match = re.search(r'Invoice\s*(?:No\.?|Number)[:\s]*(\d+)', text, re.IGNORECASE)
    if invoice_match:
        result['order_id'] = invoice_match.group(1)

    # Extract invoice date
    # Patterns: "Invoice Date: 15/01/2024" or "Date: 15 Jan 2024"
    date_match = re.search(r'(?:Invoice\s+)?Date[:\s]*(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}|\d{1,2}\s+\w+\s+\d{4})', text, re.IGNORECASE)
    if date_match:
        result['receipt_date'] = parse_date_text(date_match.group(1))
    ...
    for pattern in item_patterns:
        for match in re.finditer(pattern, text):
            ...
            if len(groups) == 5:
                ...
                full_name = f"{description} ({product_code})" if product_code else description
            else:
                # Simple pattern
            ...
            skip_terms = ['description', 'quantity', 'unit price', 'amount', 'subtotal',
                         'total', 'vat', 'carriage', 'delivery', 'postage']
    ...
    total_amounts = []
    for match in re.finditer(r'(?:Grand\s+)?Total[:\s]*£?\s*([0-9,]+\.?\d*)', text, re.IGNORECASE):
        amount = parse_amount(match.group(1))
        if amount and amount > 0:
            total_amounts.append(amount)
...
def parse_receipt_pdf(pdf_bytes: bytes, sender_domain: str = None, filename: str = None) -> Optional[dict]:
    """
