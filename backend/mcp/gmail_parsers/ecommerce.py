...
def parse_ebay_receipt(html_body: str, text_body: str, subject: str) -> dict | None:
    ...
    # Extract item name from subject (basic extraction - strip confirmation text)
    item_name = (
        subject.replace("Order confirmed:", "")
        .replace("your order is confirmed", "")
        .strip()
    )
    ...
    if html_body:
        ...
        # Extract order details from structured HTML (placeholder - full implementation needed)
        order_data = {}
    ...
    if item_name:
        ...
        item = {
            ...
            "category_hint": "Online Marketplace",
            "quantity": 1,
            "price": result.get("total_amount"),
        }
        # Extract brand from product name (not implemented yet)
        brand = None
        if brand:
            item["brand"] = brand
