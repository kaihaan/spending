...
def match_all_gmail_receipts(user_id: int = 1) -> dict:
    ...
    for receipt in unmatched_receipts:
        ...
        receipt_date = parse_receipt_date(receipt.get('receipt_date')) or parse_receipt_date(receipt.get('received_at'))
        if receipt_date:
            ...
            if best_match['confidence'] >= AUTO_MATCH_THRESHOLD:
                ...
                if success:
                    ...
                    for match in all_matches:
                        if match['transaction_id'] not in seen_txn_ids and match['confidence'] >= AUTO_MATCH_THRESHOLD:
                    ...
                    for i, match in enumerate(unique_matches):
                        ...
                        description = receipt.get('subject') or receipt.get('merchant_name') or 'Email Receipt'
                        try:
                    ...
                    matches_details.append({
                        ...
                        'auto_matched': True,
                        'total_matches': len([m for m in all_matches if m['confidence'] >= AUTO_MATCH_THRESHOLD])
                    })
...
def calculate_match_confidence(
    ...
    # Merchant matching
    merchant_match = is_merchant_match(
        receipt_merchant,
        txn_merchant,
        txn_description
    )
...
def is_merchant_match(
    ...
    if txn_merchant:
        ...
        if txn_merchant_normalized and receipt_merchant_lower in txn_merchant_normalized:
            return True
        if txn_merchant_normalized and txn_merchant_normalized in receipt_merchant_lower:
            return True
...
def parse_receipt_date(date_value) -> Optional[datetime]:
...
def parse_transaction_date(date_value) -> Optional[datetime]:
