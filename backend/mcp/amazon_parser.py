    ...
    try:
            ...
            orders_dict[order_id].append(
                {
                    ...
                    if "Order Status" in row
                    else None,
                    "shipment_status": str(row.get("Shipment Status", "")).strip()
                    if "Shipment Status" in row
                    else None,
                }
            )
        ...
        for _order_id, items in orders_dict.items():
            ...
            if len(items) == 1:
                ...
                abbreviated_names = [
                    abbreviate_product_name(item["product_name"]) for item in items
                ]
    ...
    try:
        ...
        for filename in os.listdir(data_folder):
            if filename.endswith(".csv") and (
                "amazon" in filename.lower() or "orderhistory" in filename.lower()
            ):
