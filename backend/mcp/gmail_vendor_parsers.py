...
def detect_amazon_email_type(subject: str, text_body: str) -> str:
    ...
    if any(x in subject_lower for x in ['shipped', 'dispatched', 'out for delivery', 'delivered']):
        return 'shipment'
    return 'order'
...
def parse_amazon_cancellation(soup, subject: str) -> dict:
    ...
    item_name = None
    item_match = re.search(r'cancelled successfully[:\s]*["\']?(.+?)["\']?\s*$', subject, re.IGNORECASE)
    if item_match:
...
def parse_amazon_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    if not result.get('order_id'):
        ...
        order_match = re.search(order_pattern, subject) or re.search(order_pattern, text_body or '')
        if order_match:
            result['order_id'] = order_match.group(1)
...
def extract_amazon_line_items(soup: BeautifulSoup, text: str) -> list:
    ...
    if not items:
        ...
        for elem in soup.find_all(['td', 'span', 'div']):
            ...
            if (10 < len(elem_text) < 150 and
                not re.search(r'[£$€]|\d{3}-\d{7}|order|total|subtotal|shipping|tax', elem_text.lower()) and
                elem_text not in seen_names):
...
def extract_item_from_row(cells: list) -> Optional[dict]:
    """
    ...
    for cell in cells:
        ...
        qty_match = re.search(r'(?:qty|quantity)[:\s]*(\d+)|x(\d+)', cell_text, re.IGNORECASE)
        if qty_match:
        ...
        if 10 < len(cell_text) < 200 and re.search(r'[a-zA-Z]{3,}', cell_text):
            ...
            if not re.search(r'order|total|subtotal|shipping|tax|delivery|amazon', cell_text.lower()):
                name = cell_text
...
def clean_product_name(name: str) -> str:
    ...
    cleaned = re.sub(r'\s+(Buy now|Shop now|View item)$', '', cleaned, flags=re.IGNORECASE)
...
def infer_product_description(name: str) -> Optional[str]:
    """
...
def parse_paypal_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    if subject_merchant_match:
        ...
        if 2 < len(merchant) < 50:
            ...
            result['merchant_name_normalized'] = merchant.lower().replace(' ', '_').replace('-', '_')
    ...
    if html_body:
        ...
        tx_match = re.search(r'Transaction\s*ID[:\s]*([A-Z0-9]{10,17})', text, re.IGNORECASE)
        if tx_match:
...
def extract_ebay_item_from_subject(subject: str) -> Optional[str]:
    """
...
def extract_ebay_order_details(soup: BeautifulSoup) -> dict:
    ...
    fetch_links = soup.find_all('a', href=re.compile(r'FetchOrderDetails', re.IGNORECASE))
...
def infer_ebay_description(name: str) -> Optional[str]:
    """
...
def infer_ebay_category(name: str) -> str:
    ...
    category_patterns = [
        (r'laptop|notebook|computer|thinkpad|macbook|phone|tablet|ipad|iphone|camera|keyboard|mouse|monitor', 'electronics'),
        ...
        (r'shirt|dress|pants|jacket|coat|clothing|shoes|trainers|boots|sneakers|fashion', 'clothing'),
        (r'headphone|earphone|earbud|speaker|audio|cable|charger|adapter', 'electronics'),
...
def parse_charles_tyrwhitt_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
...
def parse_mindbody_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    if business_match:
        ...
        purchase_match = re.search(r'Receipt for Your\s+(.+?)\s+Purchase', subject, re.IGNORECASE)
        if purchase_match:
            business_name = purchase_match.group(1).strip()

    result = {
        'merchant_name': business_name,
        'merchant_name_normalized': re.sub(r'[^a-z0-9]+', '_', business_name.lower()).strip('_'),
    ...
    tax_match = re.search(r'(?:incl\.?\s*)?tax\s*(?:of)?\s*£\s*([0-9,]+\.?\d*)', text, re.IGNORECASE)
    if tax_match:
...
def parse_fastspring_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
...
def parse_citizens_of_soil_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    product_match = re.search(r'What\'s coming.*?\n+(.+?)\s*×\s*\d+', text, re.IGNORECASE | re.DOTALL)
    if product_match:
        product_name = product_match.group(1).strip()
        if product_name:
            result['line_items'] = [{'name': product_name}]
...
def parse_black_sheep_coffee_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    for pattern in item_patterns:
        ...
        for match in matches:
            ...
            if item_name and len(item_name) > 2 and len(item_name) < 100:
                ...
                skip_words = ['total', 'subtotal', 'order', 'confirmation', 'amount', 'payment', 'tax', 'vat']
...
def parse_audio_emotion_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    ...
    # Extract order ID
    order_match = re.search(r'order\s*(?:#|number|:)?\s*(\d+)', subject + ' ' + text, re.IGNORECASE)
    if order_match:
        result['order_id'] = order_match.group(1)
...
def parse_novation_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_john_lewis_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_bluehost_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_cex_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_yreceipts_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    ...
    merchant_match = re.search(r'receipt from\s+(.+?)(?:\s*$|\s*-)', subject, re.IGNORECASE)
    merchant_name = merchant_match.group(1).strip() if merchant_match else 'Unknown'

    result = {
        'merchant_name': merchant_name,
        'merchant_name_normalized': merchant_name.lower().replace(' ', '_').replace("'", ''),
...
def parse_worldpay_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_designacable_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    ...
    # Extract order ID
    order_match = re.search(r'order\s*(?:#|:)?\s*(\d+)', subject + ' ' + text, re.IGNORECASE)
    if order_match:
        result['order_id'] = order_match.group(1)
...
def parse_cooksmill_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_bahai_events_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_bahai_books_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_world_of_books_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
            ...
            for row in rows:
                ...
                if len(cells) >= 2:
                    ...
                    if qty_text.isdigit() and title_text and len(title_text) >= 2:
                        ...
                        if 'shipped' not in title_text.lower() and '@' not in title_text:
                            line_items.append({
                                'name': title_text,
    ...
    # Pattern: Title\nPrice\n\nBook Name\n\nGB / VERY_GOOD\n\nQty: 1\n\n£3.50
    if not line_items:
        title_match = re.search(r'Title\s*\n\s*Price\s*\n(.+?)(?:Subtotal|Shipping address)', text, re.DOTALL)
        if title_match:
            ...
            while i < len(lines):
                ...
                if not re.match(r'^£', line) and not re.match(r'^[A-Z]{2}\s*/\s*\w+', line):
                    title = line
                    qty = 1
                    price = None
...
def parse_smol_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_cables4all_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
...
def parse_gear4music_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    if html_body:
        ...
        if amount_match:
            ...
            amount_match = re.search(r'Grand Total.*?£\s*([0-9,]+\.?\d*)', text, re.IGNORECASE | re.DOTALL)
            if amount_match:
                result['total_amount'] = parse_amount(amount_match.group(1))
...
def parse_uniqlo_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    if html_body:
        ...
        if not result.get('order_id'):
            order_match = re.search(r'Order Number[:\s]*([0-9\-]+)', subject, re.IGNORECASE)
            if order_match:
                result['order_id'] = order_match.group(1)
        ...
        while i < len(lines):
            ...
            if product_match:
                ...
                if variant and not variant.startswith('Buy') and not variant.startswith('Price'):
...
def parse_bloomling_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
...
def parse_o2_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    order_match = re.search(r'Order\s*(?:number)?[:\s]*([A-Z]{2}\d{8})', text, re.IGNORECASE)
    if order_match:
        result['order_id'] = order_match.group(1)
    ...
    grand_total_match = re.search(r'Order grand total(.*?)(?=\n\n|\Z)', text, re.IGNORECASE | re.DOTALL)
    if grand_total_match:
        ...
        for neg, val in reversed(amounts):
            ...
            if not neg and amount is not None:
                result['total_amount'] = amount
                break
            elif neg and amount is not None:
                # If only negative found, it's a credit
                result['total_amount'] = -amount
                break

    # Extract monthly charge
    monthly_match = re.search(r'Monthly\s+Charge[:\s]*£?(\d+\.?\d*)', text, re.IGNORECASE)
    if monthly_match:
...
def parse_reverb_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    if prices:
        ...
        total_matches = re.findall(r'Total\s*[\n\r\s]*£([\d,]+\.?\d*)', text, re.IGNORECASE)
        if total_matches:
            result['total_amount'] = parse_amount(total_matches[-1])  # Last match is grand total
...
def parse_guitarguitar_receipt(html_body: str, text_body: str, subject: str) -> Optional[dict]:
    """
    ...
    date_match = re.search(r'Receipt Date\s*-?\s*(\d{1,2})\s+(\w+)\s+(\d{4})', text, re.IGNORECASE)
    if date_match:
        day = int(date_match.group(1))
