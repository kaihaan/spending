...
def match_category_rule(description: str, transaction_type: str, rules: List[Dict]) -> Optional[Dict]:
    """
    ...
    for rule in rules:
        ...
        if pattern_type == 'contains':
            ...
            try:
                ...
                logger.warning(f"Invalid regex pattern in rule {rule.get('id')}: {pattern}")
                continue

        if matched:
            logger.debug(f"Category rule matched: {rule.get('rule_name')} for '{description[:50]}...'")
            return rule

    return None


def match_merchant_normalization(description: str, normalizations: List[Dict]) -> Optional[Dict]:
    """
    ...
    for norm in normalizations:
        ...
        if pattern_type == 'contains':
            ...
            try:
                ...
                logger.warning(f"Invalid regex pattern in normalization {norm.get('id')}: {pattern}")
                continue

        if matched:
            logger.debug(f"Merchant normalization matched: {norm.get('normalized_name')} for '{description[:50]}...'")
            return norm

    return None


def _match_extracted_payee(payee: str, normalizations: List[Dict]) -> Optional[Dict]:
    """
    ...
    for norm in normalizations:
        ...
        if matched:
            logger.debug(f"Payee '{payee}' matched normalization: {norm.get('normalized_name')}")
            return norm
...
def apply_rules_to_transaction(
    ...
    try:
        ...
        if payee:
            ...
            direct_debit_norms = [n for n in merchant_normalizations if n.get('source') == 'direct_debit']
            ...
            if payee_match and payee_match.get('default_category'):
                ...
                enrichment = {
                    ...
                    'subcategory': payee_match.get('normalized_name'),  # Use merchant as subcategory
                    'merchant_clean_name': payee_match.get('normalized_name'),
                    'merchant_type': payee_match.get('merchant_type'),
                    'essential_discretionary': _infer_essential_discretionary(payee_match.get('default_category')),
                ...
                logger.info(f"Applied direct debit rule for '{payee}' to transaction: {description[:50]}...")
                return enrichment
    except ImportError:
    ...
    if category_match:
        enrichment = {
            ...
            'merchant_clean_name': merchant_match.get('normalized_name') if merchant_match else None,
            'merchant_type': merchant_match.get('merchant_type') if merchant_match else None,
            'essential_discretionary': _infer_essential_discretionary(category_match.get('category')),
            'payment_method': None,
            'payment_method_subtype': None,
            'purchase_date': transaction.get('date'),
            ...
            'matched_merchant': merchant_match.get('normalized_name') if merchant_match else None
        }

        logger.info(f"Applied rule '{category_match.get('rule_name')}' to transaction: {description[:50]}...")
        return enrichment
...
def _infer_essential_discretionary(category: str) -> str:
    ...
    try:
        ...
        fallback_essential = {
            'Groceries', 'Utilities', 'Healthcare', 'Transportation',
            'Housing', 'Insurance', 'Education', 'Income', 'Transfer',
            'Interest', 'Banking Fees', 'Taxes'
        }
...
def get_enrichment_from_rules(
    ...
    for txn in transactions:
        txn_id = txn.get('id')
        result = apply_rules_to_transaction(txn, category_rules, merchant_normalizations)
