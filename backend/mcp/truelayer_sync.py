...
class SyncPerformanceTracker:
    ...
    Measures API call times, database writes, and overall throughput
    to identify bottlenecks and optimize sync performance.
    """
    ...
    def record_api_call(self, duration: float):
        """Record duration of an API call in seconds."""
        self.api_calls.append(duration)

    def record_db_write(self, duration: float):
        """Record duration of a database write in seconds."""
        self.db_writes.append(duration)

    def record_pagination(self, duration: float):
        """Record duration of a pagination cycle in seconds."""
        self.pagination_times.append(duration)
    ...
    def report(self, account_count: int, total_transactions: int):
        ...
        print("=" * 80)
        print(
            f"PERFORMANCE SUMMARY: {total_transactions} transactions from {account_count} accounts in {total_time:.1f}s"
        )
        print("=" * 80)
        ...
        if total_time > 0:
            ...
            print(f"Throughput: {throughput:.1f} transactions/min")

        if self.api_calls:
            avg_api = sum(self.api_calls) / len(self.api_calls)
            max_api = max(self.api_calls)
            total_api_time = sum(self.api_calls)
            print(
                f"API calls: count={len(self.api_calls)}, avg={avg_api:.3f}s, max={max_api:.3f}s, total={total_api_time:.1f}s"
            )

        if self.db_writes:
            avg_db = sum(self.db_writes) / len(self.db_writes)
            max_db = max(self.db_writes)
            total_db_time = sum(self.db_writes)
            print(
                f"DB writes: count={len(self.db_writes)}, avg={avg_db:.3f}s, max={max_db:.3f}s, total={total_db_time:.1f}s"
            )

        if self.pagination_times:
            avg_page = sum(self.pagination_times) / len(self.pagination_times)
            max_page = max(self.pagination_times)
            print(
                f"Pagination: pages={len(self.pagination_times)}, avg={avg_page:.3f}s, max={max_page:.3f}s"
            )

        # Calculate time breakdown
        if self.api_calls or self.db_writes:
            api_percent = (
                (sum(self.api_calls) / total_time * 100) if total_time > 0 else 0
            )
            db_percent = (
                (sum(self.db_writes) / total_time * 100) if total_time > 0 else 0
            )
            other_percent = 100 - api_percent - db_percent
            print(
                f"Time breakdown: API={api_percent:.1f}%, DB={db_percent:.1f}%, Other={other_percent:.1f}%"
            )
...
def identify_transaction_merchant(txn: dict) -> dict:
    ...
    merchant = identify_merchant(txn.get("description", ""), txn.get("merchant_name"))
...
def sync_account_transactions(
    ...
    try:
        ...
        if from_date and to_date:
            ...
            from_date_str = (
                from_date.isoformat() if isinstance(from_date, date) else from_date
            )
            ...
            print(
                f"   📅 Explicit date range: {from_date_str} to {to_date_str} ({sync_days} days)"
            )
            ...
            try:
                if last_synced_at:
                    ...
                    print(
                        f"   📅 Incremental sync: last synced {days_since_sync} days ago, fetching {sync_days} days"
                    )
                else:
                    print(
                        f"   📅 No previous sync found, fetching full {days_back} days"
                    )
            except Exception as e:
                print(
                    f"   ⚠️  Error parsing last_synced_at ({type(last_synced_at).__name__}): {e}"
                )
                ...
                try:
                    ...
                    if existing:
                        ...
                        if idx <= 3:  # Log first few duplicates
                            print(
                                f"     ⏭️  Transaction {idx}: Duplicate (id: {normalised_id})"
                            )
                        continue
                    ...
                    try:
                        ...
                        print(
                            f"     📊 Transaction {idx}: {merchant_name} (awaiting LLM enrichment)"
                        )
                    except Exception as e:
                        print(
                            f"     ⚠️  Could not identify merchant for transaction {idx}: {e}"
                        )
                    ...
                    if txn_id:
                        ...
                        if idx <= 3:  # Log first few insertions
                            print(
                                f"     ✅ Transaction {idx}: Inserted (id: {normalised_id})"
                            )
                    else:
                        error_count += 1
                        print(
                            f"     ❌ Transaction {idx}: Failed to insert (id: {normalised_id})"
                        )
        ...
        database.update_account_last_synced(
            db_account_id, datetime.utcnow().isoformat()
        )

        # Also update connection-level timestamp to track overall sync status
        database.update_connection_last_synced(
        ...
        print(
            f"✅ Sync complete: {synced_count} synced, {duplicate_count} duplicates, {error_count} errors"
        )
        return result
        ...
        if token_expires_at:
            try:
                ...
                if time_until_expiry < 300:
                    print(
                        f"   ⏰ Token expires in {time_until_expiry:.0f} seconds - refreshing..."
                    )
...
def sync_all_accounts(user_id: int, date_from: str = None, date_to: str = None) -> dict:
    """
    Sync transactions for all connected accounts of a user.

    Args:
        user_id: User ID
        date_from: Start date for sync (YYYY-MM-DD, optional)
        date_to: End date for sync (YYYY-MM-DD, optional)
    ...
    if date_from and date_to:
        print(
            f"🔄 Starting sync for user {user_id} (date range: {date_from} to {date_to})..."
        )
    else:
    ...
    for connection in connections:
        ...
        print(
            f"   📍 Connection {connection_id} - Status: {conn_status}, Provider: {provider_id}/{provider_name}"
        )
                ...
                if encrypted_token:
                    ...
                    if api_accounts:
                        ...
                        if new_provider_id or new_provider_name:
                            print(
                                f"   🔄 Updating provider info: {new_provider_id} / {new_provider_name}"
                            )
                            database.update_connection_provider(
                                connection_id, new_provider_id, new_provider_name
                            )
            except Exception as e:
        ...
        for account in accounts:
            ...
            print(
                f"     🏦 Account: {account_display_name} (ID: {truelayer_account_id}, DB ID: {db_account_id})"
            )
            ...
            try:
                ...
                access_token = decrypt_token(encrypted_token)
                print(
                    f"   ✅ Successfully decrypted access token for account {truelayer_account_id}"
                )
                ...
                result = sync_account_transactions(
                    ...
                    from_date=date_from,  # Pass date range from UI
                    to_date=date_to,  # Pass date range from UI
                )
    ...
    tracker.report(
        ...
        total_transactions=total_synced + total_duplicates,
    )
...
def sync_card_transactions(
    ...
    try:
        ...
        print(
            f"✅ Card sync complete: {synced_count} synced, {duplicate_count} duplicates, {error_count} errors"
        )
        return result
...
def sync_all_cards(user_id: int) -> dict:
    ...
    if not connections:
        print(f"⚠️  No active connections found for user {user_id}")
        return {"user_id": user_id, "total_cards": 0, "total_synced": 0, "cards": []}
    ...
    for connection in connections:
        ...
        try:
            ...
            for card in cards:
                ...
                last_four = (
                    card.get("partial_card_number", "")[-4:]
                    if card.get("partial_card_number")
                    else None
                )
...
def handle_webhook_event(event_payload: dict) -> dict:
    ...
    try:
        ...
        if event_type == "transactions_available":
            ...
            return {"event_id": event_id, "status": "processed", "sync_result": result}
        ...
        if event_type == "balance_updated":
            ...
            return {"event_id": event_id, "status": "processed", "balance": balance}
