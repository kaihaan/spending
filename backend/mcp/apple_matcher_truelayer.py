...
def match_all_apple_transactions():
    ...
    print(f"[Apple Matcher] Found {len(unmatched_transactions)} unmatched TrueLayer APPLE transactions")
    ...
    if not all_apple:
        print("[Apple Matcher] WARNING: No Apple transactions in database to match against!")
        return {
    ...
    for transaction in unmatched_transactions:
        print(f"[Apple Matcher] Checking bank txn: date={transaction.get('date')}, amount={transaction.get('amount')}, desc={transaction.get('description')[:50]}...")
        match = find_best_apple_match(transaction, all_apple)

        if match:
            print(f"[Apple Matcher] MATCH FOUND: order_id={match['order_id']}, app={match['app_names']}, confidence={match['confidence']}")
            ...
            print(f"[Apple Matcher] No match found for this transaction")

    print(f"[Apple Matcher] RESULT: {matched_count} matched, {len(unmatched_transactions) - matched_count} unmatched")
    return {
        'total_processed': len(unmatched_transactions),
...
def find_best_apple_match(transaction, apple_transactions):
    ...
    try:
        ...
        if isinstance(txn_date_val, datetime):
            ...
            txn_date = datetime.combine(txn_date_val, datetime.min.time(), tzinfo=tz.utc)
        else:
    ...
    print(f"[Apple Matcher]   Bank txn parsed: date={txn_date.date()}, amount=£{txn_amount:.2f}")
    ...
    if not ('APPLE' in merchant or 'APPLE' in description):
        print(f"[Apple Matcher]   Skipped: Not an Apple transaction")
        return None

    # Find candidates
    candidates = []
    print(f"[Apple Matcher]   Searching {len(apple_transactions)} Apple transactions for match...")
        ...
        try:
            if isinstance(apple['order_date'], datetime):
                ...
                if apple_date.tzinfo is None:
                    apple_date = apple_date.replace(tzinfo=tz.utc)
            elif isinstance(apple['order_date'], type(datetime.now().date())):  # date type
                apple_date = datetime.combine(apple['order_date'], datetime.min.time(), tzinfo=tz.utc)
            else:
                apple_date = datetime.strptime(str(apple['order_date']), '%Y-%m-%d').replace(tzinfo=tz.utc)
        except:
            continue
    ...
    if not candidates:
        ...
        print(f"[Apple Matcher]   No candidates found. Sample Apple transactions:")
        for apple in apple_transactions[:3]:  # Show first 3
            print(f"[Apple Matcher]     Apple: date={apple.get('order_date')}, amount=£{apple.get('total_amount'):.2f}, app={apple.get('app_names', '')[:30]}")
        return None
    ...
    best = candidates[0]
    print(f"[Apple Matcher]   Best candidate: {best['app_names'][:30]}, confidence={best['confidence']}")
