...
def is_ebay_receipt_email(subject: str, sender_email: str) -> tuple:
    ...
    if (
        subject_lower == "your order is confirmed."
        or subject_lower == "your order is confirmed"
    ):
...
def is_deliveroo_receipt_email(subject: str, sender_email: str) -> tuple:
    ...
    if (
        "order's in the kitchen" in subject_lower
        or "order is in the kitchen" in subject_lower
    ):
...
def is_google_receipt_email(subject: str, sender_email: str) -> tuple:
    ...
    for pattern in GOOGLE_RECEIPT_SENDERS:
        if pattern in sender_lower:
            ...
            if any(
                ind in subject_lower
                for ind in ["receipt", "invoice", "order", "payment"]
            ):
...
def extract_with_patterns(
    ...
    if sender_pattern:
        ...
        merchant_name = extract_merchant_from_text(
            subject, sender_email, sender_domain, sender_name
        )
