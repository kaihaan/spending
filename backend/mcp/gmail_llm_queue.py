...
def get_current_llm_config() -> dict:
    ...
    try:
        ...
        costs = PROVIDER_COSTS.get(
            config.provider, PROVIDER_COSTS[LLMProvider.ANTHROPIC]
        )
...
def fetch_and_parse_receipt_with_llm(receipt_id: int) -> dict:
    ...
    try:
        ...
        database.update_receipt_llm_status(
            receipt_id, status="failed", actual_cost=actual_cost
        )
...
def process_llm_queue(
    receipt_ids: list, connection_id: int = None
