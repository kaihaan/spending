...
class SyncPerformanceTracker:
    ...
    def report(self, message_count: int):
        ...
        if PSUTIL_AVAILABLE:
            try:
                memory_used = (
                    psutil.Process().memory_info().rss / 1024 / 1024 - self.start_memory
                )
            except Exception:
        ...
        logger.info("=" * 80)
        logger.info(
            f"PERFORMANCE SUMMARY: {message_count} messages in {total_time:.1f}s"
        )
        logger.info("=" * 80)
        ...
        if self.api_calls:
            logger.info(
                f"API calls: avg={sum(self.api_calls) / len(self.api_calls):.3f}s, max={max(self.api_calls):.3f}s"
            )

        if self.db_writes:
            logger.info(
                f"DB writes: avg={sum(self.db_writes) / len(self.db_writes):.3f}s, max={max(self.db_writes):.3f}s"
            )

        if self.parse_times:
            logger.info(
                f"Parsing: avg={sum(self.parse_times) / len(self.parse_times):.3f}s, max={max(self.parse_times):.3f}s"
            )
...
def should_import_email(
    subject: str, sender_email: str, body_text: str = None
) -> tuple:
    """
    ...
    if sender_domain in SHIPPING_DOMAINS:
        ...
        if (
            "thank you for your purchase" in subject_lower
            or "receipt" in subject_lower
...
def sync_receipts_full(
    ...
    logger.info(
        ...
        extra={"sync_job_id": job_id, "connection_id": connection_id},
    )
    ...
    try:
        ...
        logger.info(
            f"Gmail profile: {profile['email_address']}, history_id={latest_history_id}",
        ...
        while True:
            result = list_receipt_messages(
                service, query=query, page_token=page_token, max_results=100
            )
            ...
            page_count += 1
            logger.info(
                f"Page {page_count}: {len(messages)} messages (total: {len(all_message_ids)})",
                extra={"sync_job_id": job_id},
            )
            ...
            if len(all_message_ids) >= MAX_MESSAGES_PER_SYNC:
                logger.warning(
                    f"Hit message limit ({MAX_MESSAGES_PER_SYNC})",
                    extra={"sync_job_id": job_id},
                )
                break
        ...
        logger.info(
            f"Found {total_messages} potential receipt messages",
            extra={"sync_job_id": job_id},
        )

        # Update job with total
        database.update_gmail_sync_job_progress(job_id, total_messages, 0, 0, 0)
        ...
        for i in range(0, total_messages, BATCH_SIZE):
            ...
            access_token_batch, refresh_token_batch = get_gmail_credentials(
                connection_id
            )
            ...
            if GMAIL_PARALLEL_FETCH and len(batch_ids) > 1:
                ...
                with ThreadPoolExecutor(
                    max_workers=min(GMAIL_SYNC_WORKERS, len(batch_ids))
                ) as executor:
                            ...
                            if USE_BULK_WRITES:
                                ...
                                if prepared.get("receipt_data"):
                                    stats.record_parse_attempt(
                                        ...
                                        llm_cost_cents=prepared["receipt_data"].get(
                                            "llm_cost_cents"
                                        ),
                                    )
                                ...
                                if prepared["action"] == "store":
                                    receipt_batch.append(
                                        (
                                            prepared["connection_id"],
                                            prepared["message_id"],
                                            prepared["receipt_data"],
                                        )
                                    )
                                ...
                                if result.get("receipt_data"):
                                    stats.record_parse_attempt(
                                        ...
                                        llm_cost_cents=result["receipt_data"].get(
                                            "llm_cost_cents"
                                        ),
                                    )
            ...
            if USE_BULK_WRITES and (email_content_batch or receipt_batch):
                ...
                try:
                    logger.info(
                        f"Bulk inserting: {len(email_content_batch)} emails, {len(receipt_batch)} receipts",
                        extra={"sync_job_id": job_id},
                    )
                            ...
                            if receipt_id:
                                try:
                                    process_pdf_receipt_task.delay(
                                        ...
                                        received_date=prepared["receipt_data"].get(
                                            "received_at"
                                        ),
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"PDF task dispatch failed for receipt {receipt_id}: {e}",
                                        extra={
                                            "sync_job_id": job_id,
                                            "receipt_id": receipt_id,
                                        },
                    ...
                    logger.warning(
                        f"Bulk insert failed, falling back to individual inserts: {e}",
                        extra={"sync_job_id": job_id},
                        exc_info=(os.getenv("DEBUG_GMAIL_SYNC") == "true"),
                    )
                    ...
                    for msg in email_content_batch:
                        try:
                            ...
                            logger.warning(
                                f"Failed to save email content for {msg.get('message_id')}: {e2}",
                                extra={
                                    "sync_job_id": job_id,
                            ...
                            try:
                                error = GmailError.from_exception(
                                    ...
                                    context={
                                        "message_id": msg.get("message_id"),
                                        "operation": "save_email_content",
                                    },
                                )
                                error.log(
                                    connection_id=connection_id, sync_job_id=job_id
                                )
                            except Exception:  # Fixed: was bare except
                                pass  # Don't let error tracking crash sync
                    ...
                    for conn_id, msg_id, receipt_data in receipt_batch:
                        try:
                            ...
                            failed += 1  # CRITICAL: Increment failed counter
                            logger.warning(
                                f"Failed to save receipt for {msg_id}: {e2}",
                                extra={"sync_job_id": job_id, "message_id": msg_id},
                            )
        ...
        logger.info(
            f"Sync completed: {parsed} receipts stored, {duplicates} duplicates, {failed} failed",
            extra={"sync_job_id": job_id},
        )
...
def sync_receipts_incremental(
    connection_id: int, job_id: int = None, force_reparse: bool = False
) -> dict:
    """
    ...
    if not history_id:
        logger.warning(
            "No history ID, falling back to full sync",
    ...
    logger.info(
        "Starting incremental Gmail sync",
    ...
    try:
        ...
        if changes.get("full_sync_required"):
            ...
            for progress in sync_receipts_full(
                connection_id, force_reparse=force_reparse
            ):
                result = progress
            return result
        ...
        logger.info(
            f"Found {len(new_message_ids)} new messages since last sync",
            extra={"sync_job_id": job_id},
        )
        ...
        database.update_gmail_sync_job_progress(job_id, len(new_message_ids), 0, 0, 0)
        ...
        for msg_id in new_message_ids:
            try:
                msg = get_message_content(service, msg_id)
                result = store_pending_receipt(
                    connection_id, msg, service, force_reparse=force_reparse
                )
        ...
        logger.info(
            f"Incremental sync completed: {parsed} new receipts, {filtered} filtered",
            extra={"sync_job_id": job_id},
        )
...
def store_parsed_receipt(
    connection_id: int, message: dict, service=None, force_reparse: bool = False
) -> dict:
    """
    ...
    try:
        ...
        logger.warning(
            f"Failed to store email content: {e}", extra={"message_id": message_id}
        )
    ...
    if not parsed_data.get("total_amount") and service and message.get("attachments"):
        ...
        for attachment in attachments:
            ...
            is_invoice_filename = any(
                keyword in filename for keyword in invoice_keywords
            )
            is_known_pdf_vendor = sender_domain in pdf_vendor_domains

            if is_pdf and (
                is_invoice_filename or is_known_pdf_vendor or subject_mentions_invoice
            ):
                logger.info(
                    f"Parsing PDF invoice: {attachment.get('filename')}",
                    extra={"message_id": message_id, "merchant": sender_domain},
                )
                try:
                    # Download PDF attachment
                    pdf_bytes = get_attachment_content(
                        service, message_id, attachment.get("attachment_id")
                    )
                    ...
                    if pdf_bytes:
                        ...
                        if pdf_data and pdf_data.get("total_amount"):
                            ...
                            parsed_data["currency_code"] = pdf_data.get(
                                "currency_code", parsed_data.get("currency_code", "GBP")
                            )

                            # Also update order_id and date if PDF has better data
                            if pdf_data.get("order_id") and not parsed_data.get(
                                "order_id"
                            ):
                                parsed_data["order_id"] = pdf_data["order_id"]
                            if pdf_data.get("receipt_date") and not parsed_data.get(
                                "receipt_date"
                            ):
                                parsed_data["receipt_date"] = pdf_data["receipt_date"]
                            if pdf_data.get("line_items") and not parsed_data.get(
                                "line_items"
                            ):
                            ...
                            if parsed_data.get("parse_method"):
                                parsed_data["parse_method"] = (
                                    f"{parsed_data['parse_method']}_pdf_fallback"
                                )
                            else:
                                parsed_data["parse_method"] = "pdf_fallback"

                            logger.info(
                                f"Extracted amount from PDF: {pdf_data.get('currency_code', 'GBP')} {pdf_data['total_amount']}",
                                extra={
                                    "message_id": message_id,
                                    "merchant": sender_domain,
                                },
                            )
        ...
        if attachments:
            ...
            for attachment in attachments:
                if attachment.get("mime_type", "").startswith("application/pdf"):
                    ...
                    logger.info(
                        f"PDF attachment queued for async processing: {pdf_task_info['filename']}",
                        extra={"message_id": message_id, "merchant": sender_domain},
                    )
                    break
    ...
    if sender_domain == "translink.co.uk" and not pdf_task_info:
        ...
        if link_match:
            ...
            logger.info(
                f"Translink PDF queued for async processing: {receipt_url}",
                extra={"message_id": message_id, "merchant": sender_domain},
            )
    ...
    if pdf_task_info and receipt_id:
        try:
            ...
            # CRITICAL FIX: Track PDF task dispatch failures
            try:
                from mcp.error_tracking import ErrorStage, ErrorType, GmailError
...
def start_sync(
    ...
    logger.info(
        f"Starting {sync_type} sync for connection {connection_id}",
    ...
    result = None
    for progress in sync_receipts_full(
        connection_id, from_date=from_date, to_date=to_date
    ):
        result = progress
    return result
