...
VARIANT_PATTERNS = {
    "AIRBNB": {"provider": "Card Payment", "variant": "AIRBNB", "pattern": r"AIRBNB"},
    "Marketplace": {
        "provider": "Amazon",
        "variant": "Marketplace",
        "pattern": r"AMZNMktplace",
    },
    "Zettle": {"provider": "Apple Pay", "variant": "Zettle", "pattern": r"ZETTLE"},
    "PPOINT": {"provider": "Apple Pay", "variant": "PPOINT", "pattern": r"PPOINT_\*"},
    "LIME": {"provider": "Card Payment", "variant": "LIME", "pattern": r"LIME"},
    "SUMUP": {"provider": "Apple Pay", "variant": "SUMUP", "pattern": r"SUMUP\s*\*"},
}


def extract_provider_and_variant(
    description: str,
...
def extract_variables(description: str) -> dict[str, str | None]:
    """
    ...
    if not result["payee"]:
        ...
        match = re.search(
            r"CARD PAYMENT TO\s+([A-Za-z0-9\s\-*]+?)(?:\s+ON|\s*$|\*)", description
        )
        if match:
...
def extract_direct_debit_payee_fallback(description: str) -> dict[str, str | None]:
    """
    ...
    if match:
        ...
        if rest:
            ref_match = re.match(
                r"^([^,]+?)(?:\s*[,/]\s*MANDATE|\s*MANDATE|\s*$)", rest, re.IGNORECASE
            )
            if ref_match:
