...
def backfill_pdf_data():
    ...
    try:
        ...
        for receipt in receipts:
            ...
            if not pdf_bytes:
                print(f"  ❌ Failed to get PDF from MinIO")
                failed += 1
                continue

            # Parse PDF
            sender_domain = receipt['sender_email'].split('@')[1] if '@' in receipt['sender_email'] else ''
            pdf_result = parse_receipt_pdf(pdf_bytes, sender_domain, receipt['filename'])

            if not pdf_result or pdf_result.get('total_amount') is None:
                print(f"  ⚠️  PDF parsing failed or no amount found")
                failed += 1
                continue
