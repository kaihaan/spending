...
def test_brand_extraction():
    ...
    for receipt in receipts:
        ...
        if results[parse_method]["total"] <= 3:  # Show first 3 of each vendor
            ...
            print(
                f"{symbol} {parse_method:20s} ID {receipt_id:4d}: brand={brand_value:30s} | {subject[:40]}"
            )
    ...
    for vendor, stats in results.items():
        if stats["total"] > 0:
            pct = (
                (stats["with_brand"] / stats["total"]) * 100
                if stats["total"] > 0
                else 0
            )
            symbol = "✓" if pct >= 80 else "⚠" if pct >= 50 else "❌"
            print(
                f"{symbol} {vendor:20s}: {stats['with_brand']:3d}/{stats['total']:3d} ({pct:5.1f}%)"
            )
