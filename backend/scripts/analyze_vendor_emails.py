...
def analyze_charles_tyrwhitt():
    ...
    if email['body_html']:
        ...
        # Look for totals
        total_matches = re.findall(r'(?:Total|Grand Total|Order Total)[:\s]*£\s*([0-9,]+\.[0-9]{2})', text, re.IGNORECASE)
        print(f"Total matches: {total_matches}\n")

        # Look for order IDs
        order_ids = re.findall(r'Order\s*(?:Number|ID|#)[:\s]*([A-Z0-9\-]+)', text, re.IGNORECASE)
