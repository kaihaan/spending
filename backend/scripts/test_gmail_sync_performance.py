...
def test_sync_performance(
    connection_id: int, message_limit: int = 100, use_parallel: bool = True
):
    """
    ...
    try:
        ...
        print(f"\n{'=' * 80}")
        print("GMAIL SYNC PERFORMANCE TEST")
        print(f"{'=' * 80}")
        ...
        print("🔄 Starting sync with date range (last 30 days)...\n")
        print("   Note: Will process all messages in date range")
        ...
        for progress in sync_receipts_full(
            ...
            if progress.get("status") == "scanning":
                ...
                if total > message_limit * 2:
                    print(
                        f"⚠️  Warning: Found {total} messages (recommended: ~{message_limit})"
                    )
                    print("   This test may take longer than expected")
                    print("   Consider using a smaller date range for faster testing\n")
        ...
        print(f"{'=' * 80}")
        print("Check the PERFORMANCE SUMMARY above for detailed metrics\n")
...
def main():
    ...
    parser.add_argument(
        "--connection-id", type=int, required=True, help="Gmail connection ID to test"
    )
    parser.add_argument(
        "--limit",
        type=int,
