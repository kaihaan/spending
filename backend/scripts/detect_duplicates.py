    ...
    try:
        ...
        for i, receipt in enumerate(receipts):
            ...
            for j in range(i + 1, len(receipts)):
                ...
                if (receipt['merchant_name'] != other['merchant_name'] or
                    receipt['total_amount'] != other['total_amount']):
                    continue

                # Must be within 5 minutes (delivery/dispatch emails come seconds after order)
                if not times_close(receipt['received_at'], other['received_at'], tolerance_minutes=5):
                    continue
