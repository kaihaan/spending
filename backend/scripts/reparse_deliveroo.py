...
def reparse_deliveroo():
    ...
    try:
        ...
        for receipt in receipts:
            print(f"Re-parsing receipt ID {receipt['id']}: {receipt['subject'][:50]}...")

            # Re-parse the email
            sender_domain = receipt['sender_email'].split('@')[1] if '@' in receipt['sender_email'] else ''
            ...
            has_restaurant = any(item.get('restaurant') for item in line_items if isinstance(item, dict))

            if not has_restaurant:
                print(f"  ⚠️  No restaurant field in parsed result")
                failed += 1
                continue
            ...
            restaurant_name = line_items[0].get('restaurant', 'Unknown') if line_items else 'Unknown'
