...
def init_pool():
    ...
    try:
        ...
        if connection_pool:
            ...
            # Create category promotion tables if they don't exist
            _create_category_promotion_tables()
...
def _create_category_promotion_tables():
    """Create category promotion tables if they don't exist."""
    try:
        with get_db() as conn:
            with conn.cursor() as cursor:
                ...
                # Create subcategory_mappings table
                cursor.execute('''
                ...
                # Create indexes
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_custom_categories_user ON custom_categories(user_id)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_custom_categories_type ON custom_categories(category_type)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_subcategory_mappings_category ON subcategory_mappings(custom_category_id)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_subcategory_mappings_subcategory ON subcategory_mappings(subcategory_name)')

                conn.commit()
                print("✓ Category promotion tables created successfully")
    except Exception as e:
        print(f"⚠️  Warning: Could not create category promotion tables: {e}")
...
def get_apple_statistics():
    ...
    with get_db() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            # Matched count
            cursor.execute('SELECT COUNT(*) as count FROM truelayer_apple_transaction_matches')
...
def save_gmail_connection(user_id: int, email_address: str, access_token: str,
                          refresh_token: str, token_expires_at: str, scopes: str = None) -> int:
    """
...
def update_gmail_tokens(connection_id: int, access_token: str, refresh_token: str,
                        token_expires_at: str) -> bool:
...
def update_gmail_connection_status(connection_id: int, status: str, error: str = None) -> bool:
...
def get_gmail_receipts(connection_id: int, limit: int = 50, offset: int = 0,
...
def save_gmail_match(truelayer_transaction_id: int, gmail_receipt_id: int,
                     confidence: int, match_method: str = None,
                     match_type: str = 'standard') -> int:
        ...
        with conn.cursor() as cursor:
            cursor.execute('''
                ...
                SET match_confidence = EXCLUDED.match_confidence,
                    ...
                    matched_at = NOW()
                RETURNING id
            ''', (truelayer_transaction_id, gmail_receipt_id, confidence,
                  match_method, match_type))
            result = cursor.fetchone()
            conn.commit()
            ...
            if result:
                ...
                if gmail_data:
                    ...
                    if line_items and isinstance(line_items, list) and len(line_items) > 0:
                        ...
                        if item_names:
                            description_parts.append(': ' + ', '.join(item_names[:5]))  # Limit to 5 items
                    elif order_id:
                        # Fallback to order ID if no line items
                        description_parts.append(f' #{order_id}')

                    description = ''.join(description_parts) if description_parts else f'Receipt #{gmail_receipt_id}'
...
def update_gmail_sync_job_progress(job_id: int, total: int, processed: int,
                                   parsed: int, failed: int) -> bool:
...
def complete_gmail_sync_job(job_id: int, status: str = 'completed', error: str = None) -> bool:
...
def cleanup_stale_gmail_jobs(queued_timeout_minutes: int = 5,
                             running_timeout_minutes: int = 10) -> int:
    """
...
def get_gmail_sync_job(job_id: int) -> dict:
    ...
    with get_db() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            job['progress_percentage'] = round((processed / total * 100) if total > 0 else 0)
    ...
    with get_db() as conn:
        with conn.cursor() as cursor:
            cursor.execute('''
                ...
                WHERE id = %s
            ''', (
                merchant_name, merchant_name_normalized, order_id,
                total_amount, currency_code, receipt_date,
                json.dumps(line_items) if line_items else None,
                receipt_hash, parse_method, parse_confidence,
                parsing_status, llm_cost_cents, receipt_id
            ))
            conn.commit()
            return cursor.rowcount > 0
...
def get_unparseable_receipts_for_llm_queue(connection_id: int = None, limit: int = 100) -> list:
    """
    ...
    with get_db() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            return dict(row) if row else {
                'total_count': 0, 'available_count': 0, 'pending_count': 0,
                'processing_count': 0, 'completed_count': 0, 'failed_count': 0,
                'total_estimated_cost_cents': 0, 'total_actual_cost_cents': 0
            }
...
def update_receipt_llm_status(
    ...
    with get_db() as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                f"UPDATE gmail_receipts SET {', '.join(updates)} WHERE id = %s",
                params
            )
            conn.commit()
            return cursor.rowcount > 0
    ...
    with get_db() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            for m in merchants_raw:
                ...
                m['potential_transaction_matches'] = potential_result['count'] if potential_result else 0
                ...
                ''', (user_id, f'%{match_term}%', f'%{match_term}%'))
                alt_sources = {row['source_type']: row['count'] for row in cursor.fetchall()}
            ...
            summary = {
                ...
                'total_potential_matches': sum(m['potential_transaction_matches'] for m in merchants),
...
def get_receipts_by_domain(merchant_domain: str = None, merchant_normalized: str = None,
                           user_id: int = 1, limit: int = 50, offset: int = 0,
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            for row in cursor.fetchall():
                ...
                if receipt['receipt_date']:
                    receipt['receipt_date'] = receipt['receipt_date'].isoformat() if hasattr(receipt['receipt_date'], 'isoformat') else str(receipt['receipt_date'])
...
def save_pdf_attachment(gmail_receipt_id: int, message_id: str, bucket_name: str,
                        object_key: str, filename: str, content_hash: str,
                        size_bytes: int, etag: str = None) -> int:
