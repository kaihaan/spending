...
from config.llm_config import LLMProvider, get_provider_info, load_llm_config
...
def get_account_info() -> dict:
    ...
    llm_cfg = load_llm_config()
    if not llm_cfg:
        return {"configured": False, "error": "LLM enrichment not configured"}
...
def validate_config() -> dict:
    ...
    if not llm_cfg:
        return {"valid": False, "message": "LLM enrichment is not configured"}
...
def get_available_models() -> dict:
    ...
    all_models_response = {"current_provider": current_provider, "all_models": {}}
    ...
    for provider in LLMProvider:
        ...
        for model in supported_models:
            model_list.append(
                {"name": model, "selected": llm_cfg and llm_cfg.model == model}
            )

        # Initialize provider models structure
        all_models_response["all_models"][provider.value] = {
            "provider": provider.value,
            "selected": llm_cfg.model
            if llm_cfg and llm_cfg.provider == provider
            else None,
            "built_in": model_list,
...
def get_stats() -> dict:
    ...
    return {
        ...
        "enrichment_percentage": round(
            (enriched_count / max(total_transactions, 1) * 100), 1
        ),
        "cache_stats": {"total_cached": cache_stats.get("total_keys", 0)},
    }
...
def estimate_cost(transaction_ids: list = None, force_refresh: bool = False) -> dict:
    ...
    for txn in transactions:
        ...
        cached = enrichment.get_enrichment_from_cache(
            txn.get("description", ""), direction
        )
        if cached:
            cached_count += 1
        else:
...
def trigger_enrichment(
    ...
    if not confirm_cost:
        raise ValueError(
            "Cost confirmation required. Set confirm_cost=true to proceed."
        )
    ...
    task = enrich_transactions_task.apply_async(args=[transaction_ids, force_refresh])
...
def get_job_status(job_id: str) -> dict:
    ...
    if task.state == "PROGRESS":
        return {
            ...
            "progress_percentage": round(
                (task.info.get("current", 0) / max(task.info.get("total", 1), 1)) * 100,
                1,
            ),
        }
    ...
    if task.state == "SUCCESS":
        ...
        if isinstance(result, dict) and "stats" in result:
            return {"job_id": job_id, "status": "completed", **result.get("stats", {})}
        return result
...
def prepare_stream_enrichment(
    ...
    if not transaction_ids:
        try:
            ...
            if mode == "required":
                all_transactions = (
                    enrichment.get_required_unenriched_transactions(limit=limit) or []
                )
                ...
                if mode == "unenriched":
                    all_transactions = [
                        t for t in all_transactions if not t.get("is_enriched")
                    ]
