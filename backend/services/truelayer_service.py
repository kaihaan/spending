...
def sync_transactions(
    ...
    if date_from and date_to:
        print(
            f"🔄 Starting TrueLayer sync for user {user_id} (date range: {date_from} to {date_to})..."
        )
    else:
        print(f"🔄 Starting TrueLayer sync for user {user_id}...")

    # Sync all accounts for user with optional date range
    result = truelayer_sync.sync_all_accounts(
    ...
    total_duplicates = sum(
        acc.get("duplicates", 0) for acc in result.get("accounts", [])
    )
    total_errors = sum(acc.get("errors", 0) for acc in result.get("accounts", []))

    print(
        f"✅ Sync completed: {total_synced} synced, {total_duplicates} duplicates, {total_errors} errors"
    )
    ...
    for connection in connections:
        ...
        for account in accounts:
            truelayer.save_connection_account(
                connection_id=connection["id"], account_data=account
            )
            total_discovered += 1
...
def fetch_accounts(connection_id: int) -> dict:
    ...
    for account in accounts:
        truelayer.save_connection_account(
            connection_id=connection_id, account_data=account
        )

    return {"message": f"Fetched {len(accounts)} account(s)", "accounts": accounts}
...
def fetch_cards(connection_id: int) -> dict:
    ...
    # Save to database
    for card in cards:
        truelayer.save_connection_card(connection_id=connection_id, card_data=card)

    return {"message": f"Fetched {len(cards)} card(s)", "cards": cards}
...
def handle_webhook(
    event_id: str, event_type: str, payload: dict, signature: str = None
) -> dict:
    """
...
def get_job_status(job_id: str) -> dict:
    ...
    if task.state == "PROGRESS":
        # Extract progress metadata from task.info
        info = task.info or {}
        return {
            "job_id": job_id,
            ...
            "accounts_processed": info.get("accounts_processed", 0),
            "transactions_synced": info.get("transactions_synced", 0),
